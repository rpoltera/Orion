...
function startServer() {
  ...
  try {
    ...
    console.log('[Main] Server exists:', fs.existsSync(serverPath));

    // Use node from the bundled copy in resources, fall back to Program Files
    const bundledNode = path.join(process.resourcesPath, 'node', 'node.exe');
    const nodeExe = fs.existsSync(bundledNode)
      ? bundledNode
      : 'C:\\Program Files\\nodejs\\node.exe';

    console.log('[Main] Node exe:', nodeExe, 'exists:', fs.existsSync(nodeExe));

    serverProcess = spawn(nodeExe, [serverPath], {
      env: {
        ...process.env,
        APPDATA: app.getPath('userData'),
        ELECTRON_RUN_AS_NODE: undefined, // make sure this is NOT set
      },
    ...
    serverProcess.on('exit', (code, signal) => console.log('[Server] Exited code:', code, 'signal:', signal));
    console.log('[Main] Server spawned, pid:', serverProcess.pid);
  } catch(e) {
    console.error('[Main] startServer error:', e.message);
  }
}
...
ipcMain.handle('window-set-fullscreen', (_, flag) => mainWindow?.setFullScreen(flag));
