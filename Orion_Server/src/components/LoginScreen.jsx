...
export default function LoginScreen({ onLogin }) {
  ...
  const handleSetupFinish = async () => {
    ...
    try {
      ...
      setError('Cannot connect to Orion server.');
    }
    setLoading(false);
  };
    ...
    <div style={wrapStyle}>
      <div style={{ width: '100%', maxWidth: 480, padding: '0 24px' }}>
        <div style={{ textAlign: 'center', marginBottom: 16 }}>
          <img src="/logo.png" alt="Orion" style={{ width: 340, maxWidth: '90%', display: 'block', margin: '0 auto 0px', mixBlendMode: 'screen' }} onError={e => e.target.style.display='none'} />
            ...
            <div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <div>
                  ...
                  <input style={inputStyle} value={setupName} onChange={e => { setSetupName(e.target.value); setError(''); }} placeholder="Enter a username" autoFocus autoComplete="username" onFocus={e => e.target.style.borderColor='#0063e5'} onBlur={e => e.target.style.borderColor='rgba(255,255,255,0.12)'} onKeyDown={e => e.key==='Enter' && document.getElementById('setup-pass')?.focus()} />
                </div>
                <div>
                  ...
                  <div style={{ position: 'relative' }}>
                    <input id="setup-pass" style={inputStyle} type={showPass ? 'text' : 'password'} value={setupPass} onChange={e => { setSetupPass(e.target.value); setError(''); }} placeholder="Minimum 4 characters" autoComplete="new-password" onFocus={e => e.target.style.borderColor='#0063e5'} onBlur={e => e.target.style.borderColor='rgba(255,255,255,0.12)'} onKeyDown={e => e.key==='Enter' && document.getElementById('setup-pass2')?.focus()} />
                ...
                <div>
                  ...
                  <input id="setup-pass2" style={inputStyle} type={showPass ? 'text' : 'password'} value={setupPass2} onChange={e => { setSetupPass2(e.target.value); setError(''); }} placeholder="Repeat password" autoComplete="new-password" onFocus={e => e.target.style.borderColor='#0063e5'} onBlur={e => e.target.style.borderColor='rgba(255,255,255,0.12)'} onKeyDown={e => e.key==='Enter' && setSetupStep(3)} />
                </div>
              </div>
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
                ...
                <button style={{ ...btnPrimary, marginTop: 0 }} onClick={() => { if (!setupName.trim()) { setError('Enter a username'); return; } if (setupPass.length < 4) { setError('Password must be at least 4 characters'); return; } if (setupPass !== setupPass2) { setError('Passwords do not match'); return; } setError(''); setSetupStep(3); }}>Next →</button>
              </div>
            </div>
          )}
          {setupStep === 3 && (
            <div>
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: 20 }}>
                {AVATARS.map(a => (
                  <button key={a} onClick={() => setSetupAvatar(a)} style={{ width: 48, height: 48, fontSize: 24, borderRadius: 10, cursor: 'pointer', background: setupAvatar===a ? 'rgba(0,99,229,0.25)' : 'rgba(255,255,255,0.05)', border: `2px solid ${setupAvatar===a ? '#0063e5' : 'transparent'}`, transition: 'all 0.15s' }}>{a}</button>
                ))}
              </div>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                ...
                <button style={{ ...btnPrimary, marginTop: 0, background: loading ? '#147fcd' : '#0063e5' }} onClick={handleSetupFinish} disabled={loading}>{loading ? 'Creating account...' : '✓ Finish Setup'}</button>
              </div>
            </div>
          )}
        </div>
  ...
  if (mode === 'select' && !selected) return (
    <div style={wrapStyle}>
      <div style={{ textAlign: 'center', marginBottom: 36 }}>
        <img src="/logo.png" alt="Orion" style={{ width: 280, maxWidth: '80%', display: 'block', margin: '0 auto 16px', mixBlendMode: 'screen' }} onError={e => e.target.style.display='none'} />
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16, justifyContent: 'center', maxWidth: 600, padding: '0 24px' }}>
        {users.map(user => (
          <div key={user.id} onClick={() => setSelected(user)} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, cursor: 'pointer', padding: '20px 24px', borderRadius: 14, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', transition: 'all 0.2s', minWidth: 110 }} onMouseEnter={e => { e.currentTarget.style.background='rgba(0,99,229,0.18)'; e.currentTarget.style.borderColor='rgba(0,99,229,0.4)'; }} onMouseLeave={e => { e.currentTarget.style.background='rgba(255,255,255,0.04)'; e.currentTarget.style.borderColor='rgba(255,255,255,0.08)'; }}>
            <div style={{ width: 68, height: 68, borderRadius: '50%', background: 'linear-gradient(135deg,#1a1a3e,#2d1b69)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 30, border: '2px solid rgba(255,255,255,0.1)' }}>{user.avatar || '👤'}</div>
    ...
    <div style={wrapStyle}>
      <div style={{ width: '100%', maxWidth: 380, padding: '0 24px' }}>
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          {selected ? (
            <>
              <div style={{ width: 72, height: 72, borderRadius: '50%', background: 'linear-gradient(135deg,#1a1a3e,#2d1b69)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 32, margin: '0 auto 10px', border: '2px solid rgba(255,255,255,0.12)' }}>{selected.avatar || '👤'}</div>
              <div style={{ fontSize: 20, fontWeight: 700, color: 'white' }}>{selected.name}</div>
            </>
          ) : (
            <img src="/logo.png" alt="Orion" style={{ width: 260, maxWidth: '80%', display: 'block', margin: '0 auto 4px', mixBlendMode: 'screen' }} onError={e => e.target.style.display='none'} />
          )}
        </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            {!selected && (
              <div>
                ...
                <input style={inputStyle} value={username} onChange={e => { setUsername(e.target.value); setError(''); }} placeholder="Your username" autoFocus autoComplete="username" onFocus={e => e.target.style.borderColor='#0063e5'} onBlur={e => e.target.style.borderColor='rgba(255,255,255,0.12)'} onKeyDown={e => e.key==='Enter' && document.getElementById('login-pass')?.focus()} />
              </div>
            )}
            <div>
              ...
              <div style={{ position: 'relative' }}>
                <input id="login-pass" style={inputStyle} type={showPass ? 'text' : 'password'} value={password} onChange={e => { setPassword(e.target.value); setError(''); }} placeholder="Enter password" autoFocus={!!selected} autoComplete="current-password" onFocus={e => e.target.style.borderColor='#0063e5'} onBlur={e => e.target.style.borderColor='rgba(255,255,255,0.12)'} onKeyDown={e => e.key==='Enter' && handleLogin()} />
                <button onClick={() => setShowPass(s => !s)} style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', color: '#6b7180', cursor: 'pointer', fontSize: 16 }}>{showPass ? '🙈' : '👁'}</button>
              </div>
            </div>
          </div>
          {error && <div style={{ marginTop: 12, fontSize: 13, color: '#f87171' }}>{error}</div>}
          <button style={{ ...btnPrimary, background: loading ? '#147fcd' : '#0063e5' }} onClick={handleLogin} disabled={loading}>{loading ? 'Signing in...' : 'Sign In'}</button>
        </div>
        {(selected || mode === 'select') && users.length > 1 && (
          <button onClick={() => { setSelected(null); setPassword(''); setError(''); }} style={{ display: 'block', margin: '14px auto 0', background: 'none', border: 'none', color: '#6b7180', cursor: 'pointer', fontSize: 13 }}>← Switch User</button>
        )}
      </div>
    </div>
  );
