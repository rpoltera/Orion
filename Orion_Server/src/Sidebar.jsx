...
import {
  ...
  Search, Zap, HardDrive, Plus, Wand2, Grid, Users, Clock } from 'lucide-react';

export default function Sidebar() {
  const { activeSection, setActiveSection, library, libraryCounts, iptvChannels, search, searchQuery, hardwareInfo, currentUser, setCurrentUser, scanStatus, probeStatus } = useApp();
  ...
  return (
    <nav className="sidebar">
      ...
      <div className="sidebar-nav">
        ...
        {hasMovies      && navItem('movies',      Film,  'Movies',       libraryCounts.movies      || library.movies?.length)}
        {hasTVShows     && navItem('tvshows',      Tv,    'TV Shows',     libraryCounts.tvShows     || tvSeriesCount)}
        {hasMusic       && navItem('music',        Music, 'Music',        libraryCounts.music       || library.music?.length)}
        {hasMusicVideos && navItem('musicvideos',  Video, 'Music Videos', libraryCounts.musicVideos || library.musicVideos?.length)}

        {/* Add Library shortcut */}
        {!hasAnyLibrary && (
          <div className="sidebar-item" onClick={() => setActiveSection('settings')}
        ...
        <div
          ...
          onClick={() => setActiveSection('streamforge')}
          style={{ gap: 8 }}
        >
          <img
        ...
        {[
          ...
          <div key={s.id} className={`sidebar-item ${activeSection === s.id ? 'active' : ''}`} onClick={() => setActiveSection(s.id)}>
        ...
        {navItem('autocollections',   Wand2,    'Auto Collections',   null)}
        {navItem('scheduler',          Clock,    'Scheduler',          null)}
      ...
      {probeStatus && (
        <div style={{ padding:'10px 12px', borderTop:'1px solid var(--border)',
          background: probeStatus.justFinished ? 'rgba(16,185,129,0.12)' : 'rgba(245,158,11,0.12)' }}>
          <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:6 }}>
            {probeStatus.justFinished ? (
              <span style={{ fontSize:12 }}>✓</span>
            ) : (
              <div style={{ width:12, height:12, border:'2px solid #f59e0b',
                borderTopColor:'transparent', borderRadius:'50%', flexShrink:0,
                animation:'spin 0.8s linear infinite' }} />
            )}
            <span style={{ fontSize:11, fontWeight:600,
              color: probeStatus.justFinished ? '#10b981' : '#f59e0b',
              textTransform:'uppercase', letterSpacing:0.5 }}>
              {probeStatus.justFinished ? 'Probe Complete' : 'Probing Library'}
            </span>
          </div>
          {!probeStatus.justFinished && (
            <>
              ...
              <div style={{ height:3, background:'rgba(255,255,255,0.1)', borderRadius:2, marginBottom:5 }}>
                <div style={{
                  ...
                  width: `${Math.round((probeStatus.done / probeStatus.total) * 100)}%`,
                  transition:'width 0.3s ease',
                }} />
              </div>
              <div style={{ display:'flex', justifyContent:'space-between', fontSize:10, color:'rgba(255,255,255,0.4)' }}>
                <span>{probeStatus.done?.toLocaleString()} / {probeStatus.total?.toLocaleString()}</span>
                <span>{Math.round((probeStatus.done / probeStatus.total) * 100)}%</span>
              </div>
              {probeStatus.errors > 0 && (
                <div style={{ fontSize:10, color:'rgba(255,100,100,0.7)', marginTop:2 }}>
                  {probeStatus.errors} errors
                </div>
              )}
            </>
          )}
